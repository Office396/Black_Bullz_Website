      ...
      <div className="pt-16">
        ...
        <section className="relative overflow-hidden py-20 px-4">
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
            ...
            {/* Right card */}
            <div className="rounded-2xl p-6 border relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(157,78,221,0.12), rgba(18,11,34,0.9))", borderColor: "rgba(157,78,221,0.3)" }}>
              ...
              <div className="mb-4">
                ...
                <h3 className="text-2xl font-black text-white">No limits. No ads.</h3>
                <p className="text-gray-400 mt-1">Just play.</p>
              </div>
              <div className="space-y-3 mt-6">
                {[
                  ...
                  <div key={item.label} className="flex items-start gap-3 p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.04)" }}>
            ...
            <div className="flex items-center justify-center gap-2 mb-8">
              {([1, 2, 3] as const).map((s) => (
                <div key={s} className="flex items-center gap-2">
                  <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all", step >= s ? "text-white" : "text-gray-500 border border-white/10")} style={step >= s ? { background: "linear-gradient(135deg, #9d4edd, #7b2cbf)" } : {}}>
                    {s}
                  </div>
                  <span className={cn("text-sm hidden sm:block", step >= s ? "text-white" : "text-gray-500")}>
                    {s === 1 ? "Amount" : s === 2 ? "Details" : "Currency"}
                  </span>
                  {s < 3 && <div className="w-8 h-px mx-1" style={{ background: step > s ? "#9d4edd" : "rgba(255,255,255,0.1)" }} />}
                </div>
              ))}
            </div>

            <div className="rounded-2xl border p-6" style={{ background: "linear-gradient(180deg, rgba(18,11,34,0.9), rgba(9,5,20,0.9))", borderColor: "rgba(157,78,221,0.2)" }}>
              <h2 className="text-xl font-bold text-white mb-1">Support with Crypto, PayPal or Card</h2>
              <p className="text-gray-400 text-sm mb-6">Fast, secure, and private — perfectly aligned with our mission for gaming freedom.</p>

              {step === 1 && (
                <>
                  <h3 className="text-white font-semibold mb-3">Select Donation Amount</h3>
                  <div className="flex flex-wrap gap-3 mb-6">
                    {presetAmounts.map((a) => (
                      <button key={a} onClick={() => { setAmount(a); setCustomAmount("") }}
                        className={cn("px-6 py-3 rounded-xl font-semibold transition-all hover:scale-105", amount === a ? "text-white" : "text-gray-300 hover:text-white border border-white/10 hover:border-purple-500/40")}
                        style={amount === a ? { background: "linear-gradient(135deg, #9d4edd, #7b2cbf)" } : { background: "rgba(255,255,255,0.04)" }}>
                        ${a}
                      </button>
                    ))}
                    <div className="flex items-center gap-2 px-4 py-3 rounded-xl border" style={{ background: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.1)" }}>
                  ...
                  <div className="rounded-xl p-4 mb-6 border" style={{ background: "rgba(157,78,221,0.08)", borderColor: "rgba(157,78,221,0.2)" }}>
                    <h4 className="text-white font-semibold mb-3 text-sm">Why Your Support Matters</h4>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        ...
                        <div key={label} className="text-center p-3 rounded-lg" style={{ background: "rgba(255,255,255,0.04)" }}>
              ...
              {step === 2 && (
                <>
                  <h2 className="text-xl font-bold text-white mb-1">Your Details & Payment Method</h2>
                  <p className="text-gray-400 text-sm mb-5">Tell us a bit about yourself and choose how to pay.</p>

                  {/* Amount summary */}
                  <div className="rounded-xl p-4 mb-5 text-center border" style={{ background: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>
                    <p className="text-gray-400 text-sm mb-1">Your donation amount</p>
                    <p className="text-4xl font-black text-white">${finalAmount}</p>
                  </div>

                  {/* Name */}
                  <div className="mb-4">
                    <label className="block text-white text-sm font-semibold mb-2">Your Name <span className="text-gray-500 font-normal">(Optional)</span></label>
                    <input
                      type="text"
                  ...
                  {/* Message */}
                  <div className="mb-5">
                    <label className="block text-white text-sm font-semibold mb-2">Message <span className="text-gray-500 font-normal">(Optional)</span></label>
                    <textarea
                      placeholder="Add a message to your donation"
                      value={donorMessage}
                      onChange={(e) => setDonorMessage(e.target.value)}
                      rows={3}
                      className="w-full px-4 py-3 rounded-xl text-white placeholder-gray-500 focus:outline-none resize-none transition-colors"
                      style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}
                      onFocus={(e) => e.currentTarget.style.borderColor = "rgba(157,78,221,0.6)"}
                      onBlur={(e) => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"}
                    />
                      ...
                      return (
                        <div key={m.id}>
                          <button
                            ...
                            style={{
                              background: selected ? "rgba(157,78,221,0.1)" : "rgba(255,255,255,0.03)",
                              borderColor: selected ? "rgba(157,78,221,0.7)" : "rgba(255,255,255,0.08)",
                            }}
                          >
                            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-black text-lg flex-shrink-0" style={{ background: m.iconBg }}>
                              {m.icon}
                            </div>
                          ...
                          {m.id === "Crypto" && selected && (
                            <div className="mt-2 ml-2 p-3 rounded-xl border" style={{ background: "rgba(249,115,22,0.06)", borderColor: "rgba(249,115,22,0.2)" }}>
                              <p className="text-gray-400 text-xs mb-2">Select cryptocurrency</p>
                              ...
                              {cryptoCoin === "Other" && (
                                <div className="mt-3">
                                  <input
                                    ...
                                    className="w-full px-4 py-2.5 rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none transition-colors"
                                  ...
                                  />
                                  <p className="text-gray-500 text-xs mt-1.5">We'll provide a wallet address for your chosen coin on the next step.</p>
                                </div>
                              )}
                            </div>
                          )}
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => goToStep(1)} className="flex items-center gap-2 px-6 py-3 rounded-xl border text-gray-300 hover:text-white transition-colors font-semibold" style={{ borderColor: "rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)" }}>
                      ← Back
                    </button>
                    <button
                ...
                return (
                  <>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="flex -space-x-2">
                        {["#f97316","#627eea","#26a17b","#f3ba2f"].map((c,i) => (
                          <div key={i} className="w-7 h-7 rounded-full border-2 border-[#120b22] flex items-center justify-center text-xs font-black text-white" style={{ background: c }}>
                            {["₿","Ξ","₮","B"][i]}
                          </div>
                        ))}
                        <div className="w-7 h-7 rounded-full border-2 border-[#120b22] flex items-center justify-center text-xs text-gray-400" style={{ background: "rgba(255,255,255,0.1)" }}>+{moreCoinsList.length}</div>
                      </div>
                    </div>
                    <h2 className="text-xl font-bold text-white mb-1">Choose Cryptocurrency</h2>
                    <p className="text-gray-400 text-sm mb-4">Select your preferred crypto to complete this payment</p>
                    <div className="grid grid-cols-3 gap-3 mb-5 p-3 rounded-xl border text-center" style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.07)" }}>
                      <div>
                        <p className="text-white font-black text-lg">{popularCoins.length + moreCoinsList.length}</p>
                      ...
                      <div className="border-x" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
                        ...
                        <p className="text-gray-500 text-xs">Extra Fees</p>
                      </div>
                      <div>
                        <p className="text-white font-black text-lg">~1m</p>
                        <p className="text-gray-500 text-xs">Confirm Time</p>
                      </div>
                    </div>
                    <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "#9d4edd" }}>
                      Popular Currencies
                      {pricesLoading && <span className="ml-2 text-gray-500 normal-case font-normal">fetching live prices...</span>}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                      {popularCoins.map((coin) => (
                        <button key={coin.id} onClick={() => { setSelectedCryptoForPayment(coin.id); navigateToPayment(coin.id) }}
                          className="flex items-center gap-3 p-3 rounded-xl border text-left transition-all hover:border-purple-500/50 hover:bg-white/5"
                          style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.07)" }}>
                          <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-black text-base flex-shrink-0" style={{ background: coin.color }}>
                            {coin.icon}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-gray-500 text-xs">{coin.symbol} · {getDisplayPrice(coin.id, coin.price)}</p>
                          </div>
                    ...
                    <button onClick={() => setShowMoreCoins(!showMoreCoins)}
                      className="w-full flex items-center justify-between p-3 rounded-xl border mb-2 transition-colors hover:bg-white/5"
                      style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.07)" }}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <p className="text-white text-sm font-semibold">More Currencies</p>
                          <p className="text-gray-500 text-xs">{moreCoinsList.length} additional options</p>
                        </div>
                      </div>
                      <span className="text-gray-400">{showMoreCoins ? "∧" : "∨"}</span>
                    </button>
                    {showMoreCoins && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                        {moreCoinsList.map((coin) => (
                          <button key={coin.id} onClick={() => { setSelectedCryptoForPayment(coin.id); navigateToPayment(coin.id) }}
                            className="flex items-center justify-between p-3 rounded-xl border text-left transition-all hover:border-purple-500/50 hover:bg-white/5"
                            style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.07)" }}>
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-black text-sm flex-shrink-0" style={{ background: coin.color }}>
                                {coin.icon}
                              </div>
                              <span className="text-white font-semibold text-sm">{coin.name}</span>
                            </div>
                            <span className="text-gray-500 text-xs font-mono">{coin.symbol}</span>
                          </button>
                        ))}
                      </div>
                    )}
                    <button onClick={() => goToStep(2)} className="w-full mt-2 py-2 text-gray-500 text-sm hover:text-gray-300 transition-colors">
                      ← Back
                    </button>
                  </>
                )
        ...
        <section className="py-12 px-4">
          <div className="max-w-3xl mx-auto">
            <div className="rounded-2xl border p-6" style={{ background: "linear-gradient(180deg, rgba(18,11,34,0.9), rgba(9,5,20,0.9))", borderColor: "rgba(157,78,221,0.2)" }}>
              <h2 className="text-xl font-bold text-white mb-1">Community Impact</h2>
              <p className="text-gray-400 text-sm mb-6">Every dollar is allocated transparently. No hidden costs, no corporate overhead.</p>
              ...
              <blockquote className="border-l-2 pl-4 italic text-gray-400 text-sm" style={{ borderColor: "#9d4edd" }}>
                ...
                <footer className="mt-2 text-gray-500 not-italic">— Bullz Commuinity, Admin</footer>
              </blockquote>
            </div>
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="rounded-2xl border p-6 flex flex-col" style={{ background: "linear-gradient(135deg, rgba(249,115,22,0.08), rgba(18,11,34,0.9))", borderColor: "rgba(249,115,22,0.25)" }}>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="text-white font-bold">Patreon</h3>
                  ...
                  {[
                    ...
                    <div key={tier} className="p-3 rounded-xl border" style={{ background: "rgba(255,255,255,0.04)", borderColor: "rgba(249,115,22,0.15)" }}>
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white font-semibold text-sm">{tier}</span>
                        <span className="text-orange-400 font-bold text-sm">{price}</span>
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {perks.map((p) => <span key={p} className="text-xs text-gray-400 bg-white/5 px-2 py-0.5 rounded-full">{p}</span>)}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <section className="py-10 px-4">
          <div className="max-w-4xl mx-auto">
            <div className="rounded-2xl border p-6 grid grid-cols-1 sm:grid-cols-3 gap-6 text-center" style={{ background: "linear-gradient(135deg, rgba(157,78,221,0.1), rgba(18,11,34,0.9))", borderColor: "rgba(157,78,221,0.25)" }}>
              <div>
                <p className="text-4xl font-black text-white mb-1">${statsRaised}</p>
              ...
              <div className="sm:border-x" style={{ borderColor: "rgba(157,78,221,0.2)" }}>
                <p className="text-4xl font-black text-white mb-1">{statsSupporters}</p>
                <p className="text-gray-400 text-sm">Total Supporters</p>
                <p className="text-xs mt-1 text-gray-500">And growing every week</p>
              </div>
              <div>
                <p className="text-4xl font-black text-white mb-1">{statsCountries}</p>
                <p className="text-gray-400 text-sm">Countries</p>
                <p className="text-xs mt-1 text-gray-500">A truly global community</p>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-3xl mx-auto">
            <div className="rounded-2xl border p-6" style={{ background: "linear-gradient(180deg, rgba(18,11,34,0.9), rgba(9,5,20,0.9))", borderColor: "rgba(157,78,221,0.2)" }}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-xl font-bold text-white">Hall of Champions</h2>
                  <p className="text-gray-500 text-sm">The legends keeping this platform alive</p>
                </div>
              </div>
              <div className="space-y-2">
                {activeDonors.map((donor) => (
                  <div key={donor.rank} className="flex items-center gap-4 p-3 rounded-xl transition-colors hover:bg-white/5" style={{ background: "rgba(255,255,255,0.03)" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-gray-500 text-xs">{donor.country} Anonymous Supporter</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-sm" style={{ color: "#c77dff" }}>${donor.amount}</p>
                      {donor.rank <= 3 && (
                        <p className="text-xs text-gray-500">{donor.rank === 1 ? "Top Donor" : donor.rank === 2 ? "2nd Place" : "3rd Place"}</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <section className="py-16 px-4">
          <div className="max-w-3xl mx-auto text-center">
            <div className="rounded-2xl border p-10 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(157,78,221,0.15), rgba(9,5,20,0.95))", borderColor: "rgba(157,78,221,0.35)" }}>
              <div className="absolute inset-0 pointer-events-none">
              ...
              <div className="relative z-10">
                <h2 className="text-3xl font-black text-white mb-3">Join Our Revolution</h2>
        ...
        <section className="py-12 px-4">
          <div className="max-w-3xl mx-auto">
            <h2 className="text-2xl font-black text-white mb-6 text-center">Common Questions</h2>
            <div className="space-y-2">
              {faqItems.map((item, index) => (
                <div key={index} className="rounded-xl border overflow-hidden" style={{ borderColor: "rgba(157,78,221,0.15)", background: "rgba(255,255,255,0.03)" }}>
