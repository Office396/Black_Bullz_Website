...
export async function createUser(data: {
  ...
  const { data: user, error } = await supabase
    .from('users')
    .insert({ name: data.name, username: data.username, email: data.email, password_hash })
    .select()
    .single()
...
export async function toggleFavourite(userId: string, gameId: number): Promise<boolean> {
  const { data: existing } = await supabase
    .from('user_favourites').select('id').eq('user_id', userId).eq('game_id', gameId).single()
  if (existing) {
    await supabase.from('user_favourites').delete().eq('user_id', userId).eq('game_id', gameId)
    return false
  } else {
...
export async function addWatchHistory(userId: string, gameId: number): Promise<void> {
  await supabase.from('user_watch_history')
    .upsert({ user_id: userId, game_id: gameId, viewed_at: new Date().toISOString() }, { onConflict: 'user_id,game_id' })
...
export async function getWatchHistory(userId: string): Promise<{ game_id: number; viewed_at: string }[]> {
  const { data } = await supabase
    .from('user_watch_history').select('game_id, viewed_at').eq('user_id', userId).order('viewed_at', { ascending: false })
  return data || []
}
